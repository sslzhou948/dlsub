...
> 最后更新：2026-05-17 | 当前阶段：main 分支，预翻译队列已上线，待实机验证
...
```
Phase:    实机调试（main 分支，持续迭代）
状态:     核心功能可用 + 预翻译队列已合并（2026-05-17）

已确认可用：
  - ✅ 翻译功能正常（deepseek-chat）
  - ✅ 双语按钮 + 控制面板
  - ✅ 中文译文沉浸式注入（紧随英文字幕）

已修复（实机 bug）：
  - ✅ Options 测试连接按钮（绿色成功/红色失败，10s 超时）
  - ✅ API 失败时字幕区显示错误提示（5s 自动消失）
  - ✅ 字幕背景色（黑灰半透明，CSS 变量预留配置接口）
  - ✅ "前往设置"跳转（改由 Service Worker 调用 openOptionsPage）
  - ✅ 字幕排版挤压（display:block + width:fit-content 防 flex 压缩）

新功能（已合并）：
  - ✅ 预翻译队列（PrefetchQueue）— 字幕出现时超前预取后续 3 条译文
      · CC 开启时扫描 video.textTracks，cache miss 则提前发翻译请求
      · in-flight 去重、失败静默、路由/页面卸载时均调用 clear() 清理
      · 常数 PREFETCH_LOOKAHEAD=3 可调整预取条数

待推进：
  - [ ] 实机验证预翻译效果（字幕是否 0 延迟显示）
  - [ ] 与原生 CC 解耦（暂缓，见下方调研结论）

## 原生 CC 解耦调研结论（2026-05-17，暂缓）
...
> 下次会话：实机验证预翻译效果，或推进 CC 解耦（见下方调研记录）。
